...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!donor) {
      return res.status(403).json({
        error: 'Create your account before starting Stripe checkout.',
      });
    }

    if (!donor.hasPassword) {
      return res.status(403).json({
        error: 'Set up your dashboard password before starting Stripe checkout.',
      });
    }

    const baseUrl = appSettings.publicBaseUrl || `${req.protocol}://${req.get('host')}`;
    const successUrl = `${baseUrl}/share/${req.params.token}?stripe_success=true`;
    const cancelUrl = `${baseUrl}/share/${req.params.token}?stripe_canceled=true`;
    ...
    try {
      ...
      logger.error('Failed to create Stripe checkout from share link', {
        shareLinkId: shareLink.id,
        error: err && err.message,
      });
      return res.status(502).json({
