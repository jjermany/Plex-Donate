...
const RESPONSE_FIELD_IGNORE_KEYS = new Set([
  'errors',
  'error',
  'message',
  'messages',
...
function normalizeInviteCodeCandidate(value) {
...
function sanitizeInviteUrlCandidate(value) {
  if (value === undefined || value === null) {
    return null;
  }
  ...
  if (typeof value === 'number') {
    const fromNumber = String(value);
    return fromNumber ? fromNumber.trim() : null;
  }

  return null;
}

function extractInviteFieldFromResponse(
  target,
  propertyNames,
  sanitizer,
  visited = new Set()
  ...
  if (Array.isArray(target)) {
    for (const entry of target) {
      const normalized = extractInviteFieldFromResponse(
        entry,
        propertyNames,
        sanitizer,
        visited
  ...
  for (const property of propertyNames) {
    ...
    const normalized = sanitizer(target[property]);
    if (normalized) {
      return normalized;
    }
  }
  ...
  for (const [key, value] of Object.entries(target)) {
    ...
    if (RESPONSE_FIELD_IGNORE_KEYS.has(key)) {
      continue;
    }
    const normalized = extractInviteFieldFromResponse(
      value,
...
function extractInviteCodeFromResponse(data) {
  return extractInviteFieldFromResponse(
    data,
    [
      'code',
      'invite_code',
    ...
    normalizeInviteCodeCandidate
  );
}

function extractInviteUrlFromResponse(data) {
  return extractInviteFieldFromResponse(
    data,
    ['url', 'invite_url', 'inviteUrl', 'link', 'full_url', 'href', 'location'],
    sanitizeInviteUrlCandidate
  );
}
...
async function createInvite(requestOptions = {}, overrideSettings) {
  ...
  const normalizedInviteCode = extractInviteCodeFromResponse(data);
  const inviteCode =
    normalizedInviteCode || sanitizedRequestedInviteCode || requestedInviteCode;
  const inviteUrlString = extractInviteUrlFromResponse(data) || '';
